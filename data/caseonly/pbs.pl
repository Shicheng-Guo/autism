...
my $curr_dir = $dir;
    ...
    print OUT "plink --bfile All_samples_Exome_QC --covar All_samples_Exome_QC.cov --linear mperm=50000 --allow-no-sex --ci 0.95 --seed 6377474 --keep case.ind --maf 0.01 --pheno All_samples_Exome_QC.phen --mpheno $i --out $phen[$i+1].nonage\n";       
    close(OUT);
